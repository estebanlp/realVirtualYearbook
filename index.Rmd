...
h1<-gvisHistogram(data.frame(data["months_total"]),options=list(width=800, height=300))
cat(unlist(h1$html$chart))
print(h1,tag="chart")
```
...
require(doBy)
require(ggplot2)
df<-data.frame(summaryBy(months_total~research_1 + working_papers,data=data))
...
a<-ggplot(df,aes(x=working_papers, y=months_total.mean, group=research_1))
a + geom_bar(stat="identity", position="dodge",aes(fill=research_1))+labs(y="Months in REAL", x="Number of Working Papers", fill="") + theme(legend.position="right")
```
